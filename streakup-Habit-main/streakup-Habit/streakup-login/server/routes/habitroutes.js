...
// GET /api/habits/progress-weekly
// Returns: { success: true, data: [ { habitId, title, dailyCounts: [ { date: 'YYYY-MM-DD', count: n }, ...7days ], streak } ] }

router.get('/progress-weekly', userAuth, async (req, res) => {
  try {
    // Get user's habits
    const habits = await Habit.find({ userId: req.userId }).lean();

    // Build last 7 days array (strings YYYY-MM-DD, oldest first)
    const days = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    const result = habits.map(h => {
      // completionHistory expected to be array of ISO dates (or dates)
      const history = Array.isArray(h.completionHistory) ? h.completionHistory : [];

      // Normalize history to YYYY-MM-DD strings
      const normalized = history.map(dt => {
        try {
      ...
      const dailyCounts = days.map(day => {
        const count = normalized.filter(d => d === day).length;
